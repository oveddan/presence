...
void setServoValue(int servo, int servoX) {
...
int currentServoQuarters = minServoQuarters;


void draw()
{
  currentServoQuarters++;
  if (currentServoQuarters > maxServoQuarters)
    currentServoQuarters = minServoQuarters;
    
  background(percentage);
    //delay(1000);
   for(int i = 0; i < numServos; i++) {
     setServoValue(i, currentServoQuarters);
   }
}